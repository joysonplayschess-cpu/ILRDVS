...
def _tesseract_ocr_image(img, lang: str = "auto", psm: int = 4) -> dict:
    ...
    if lang == "auto":
        try:
            ...
            if n_words < LOW_WORD_RESCUE and used_psm != 3:
                retry = _select_auto_result(img, 3)
                if retry[4] > n_words:
                    data, used_psm, langs, detected, n_words, avg_raw = retry
        except pytesseract.TesseractError:
            langs = _available_langs("eng")
        ...
        try:
            data, n_words, avg_raw = _run_once(img, langs, psm)
            used_psm = psm
            if n_words < LOW_WORD_RESCUE and psm != 3:
...
_ENGINES = {
    "bhashini": _bhashini_ocr_image,
    "tesseract": _tesseract_ocr_image,
    "paddleocr": ocr_paddle.ocr_image,
    "paddle": ocr_paddle.ocr_image,
}
_SKIPPABLE = (ocr_paddle.PaddleUnavailable,)
...
def run_ocr(img, lang: str = "auto", psm: int = 4) -> dict:
    ...
    for name in priority:
        engine_fn = _ENGINES.get(name)
        if engine_fn is None:
            continue
        try:
            if name == "tesseract":
                result = engine_fn(img, lang=lang, psm=psm)
            else:
                result = engine_fn(img, lang=lang)
        except _SKIPPABLE as exc:
            attempted.append(f"{name}: skipped ({exc})")
            last_error = exc
            continue
            ...
            continue
        if result is None:
            attempted.append(f"{name}: skipped (unavailable)")
            continue
        result["engine_attempts"] = attempted + [f"{name}: used"]
        return result
    raise last_error or RuntimeError("no OCR engine available")
